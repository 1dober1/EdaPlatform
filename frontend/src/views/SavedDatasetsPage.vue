...
async function handleDownload(dataset) {
  try {
    ...
    a.href = url
    a.download = dataset.name || `dataset_${dataset.id}.csv`
      ...
      <div v-for="dataset in datasets" :key="dataset.id" class="dataset-card">
        ...
        <div class="dataset-card__content">
          <h3 class="dataset-card__title">
            {{ dataset.name || dataset.file.split('/').pop() }}
            <span v-if="dataset.file" class="dataset-ext">{{ dataset.file.split('.').pop().toLowerCase() }}</span>
          </h3>
        ...
        <div class="dataset-card__actions">
          ...
          <button class="action-btn" @click="handleDownload(dataset)" title="Скачать файл">
